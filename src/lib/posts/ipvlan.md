...
the available IPs as requests come in.

Deploying the proxy as a standalone process on a single host would be trivial, but
I built Rivenbot with Docker Swarm across several machines.
To "just deploy the proxy on the host" wasn't really on the table.
...
combined with network "gateway" bridges created by Swarm on each node. Without these
two constructs a container would have no way to reach the outside world.
These two networks create a behavior called a _Routing Mesh_.
The routing mesh itself makes it easy for outside clients to
...
it easy to access, for example, a web-server container that's replicated
...
`http://nginx:[port]` and the DNS would resolve to the appropriate Nginx container.
...
This is a big oversimplification of how a routing mesh works, but hopefully
this gets the point across that this behavior simplifies headaches when
it comes to distributed networking in combination with Docker containers.
This is exactly the convenience I wasn't willing to give up,
which meant my original plan for the proxy needed to go in the trash.
...
Months later, after some thorough research and very well-crafted prompts (not really)
...
The big trade-off for this, something that I discovered while preparing my Swarm
cluster, is that both IPvlan and Macvlan are known as "locally scoped" drivers,
something that I'll talk about later.
